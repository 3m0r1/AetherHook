...
void Engine::SetLockAndWait() {
    while (Engine::Lock.test_and_set(std::memory_order_acquire)) {
        _mm_pause();
    }
}

void Engine::ClearLock() {
    Engine::Lock.clear(std::memory_order_release);
}
...
EngineResult Engine::SetBreakPoint(HANDLE thread, PVOID address, Drx drx) {
    PCONTEXT threadCtx = Utils::GetThreadContext(thread);
